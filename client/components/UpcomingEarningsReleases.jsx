...
export const UpcomingEarningsReleases = (props) => {
    ...
    const [loadingEarningsReleases, setLoadingEarningsReleases] =
        useState(false);
    ...
    useTracker(() => {
        if (user || settings) {
            const daysToAdd = user
                ? 10
                : settings.clientSettings.upcomingEarningsReleases
                      .numberOfDaysFromTodayForEarningsReleasesPublicationIfNoUser;
    ...
    const setSelected = (stock, releases = earningsReleases) => {
        const releasesPerStock = stock
            ? releases.filter((e) => e.symbol === stock)
            : [];
        setSelectedStock(stock);
        setRelevantEarningsReleases(releasesPerStock);
    };

    const toggleCompanyConfirmedOnly = () =>
        setCompanyConfirmedOnly(!companyConfirmedOnly);
    ...
    return (
        <div>
                ...
                <div>
                    {groupedEarningsReleases.map((group, index) => {
                        ...
                        return (
                            <div key={index}>
                                <h5>
                                    {group[0].fullTimeOfDayDescription} (
                                    {uniqueRel.length})
                                </h5>
                                <div
                                    style={{
                                    ...
                                    className="earningsButtons"
                                >
                                    {uniqueRel.map((e) => {
                                        const { countRatingChanges } = e;
            ...
            {earningsReleases.length ? null : (
                <h3>there are no earnings releases.</h3>
            )}
            {relevantEarningsReleases.length ? (
