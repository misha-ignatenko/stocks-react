...
function RegressionPerformance({ symbol }) {
    const [regressionPerformance, setRegressionPerformance] = useState(undefined);
    ...
    const regrStartPxActual = _.find(data.px, p => p.dateString === regrStartDate)?.adjClose;
    const actualStartPrice = data.actualStart.adjClose;
    const actualEndPrice = data.actualEnd.adjClose;
    const actualPct = ((actualEndPrice - actualStartPrice) / actualStartPrice * 100).toFixed(2);
    ...
    // Avg
    const avgStart = altAvg && _.find(altAvg, avgPrediction =>
    ...
    // Wgt
    const altRegrStart = altWgt && _.find(altWgt, regrPrediction =>
        regrPrediction.dateString === data.actualStart.dateString
    )?.price.toFixed(2);
    const altRegrEnd = altWgt && altWgt[altWgt.length - 1].price.toFixed(2);
    const altRegrPct = ((altRegrEnd - actualStartPrice) / actualStartPrice * 100).toFixed(2);
    const altRegrPct2 = pctDiff(altRegrStart, altRegrEnd);

    const graphData = [{
        symbol: symbol,
        historicalData: data.px,
        avgAnalystRatingsEveryDay: _.map(data.avgRatingsExtended, r => ({ ...r, avg: r.rating })),
        weightedAnalystRatingsEveryDay: _.map(data.wgtRatingsExtended, r => ({ ...r, weightedRating: r.rating })),
    ...
    return (
        <div className="row">
            ...
            <p>
                If you ran a regression on <b>{data.actualStart.dateString}</b> (avg rating{' '}
                <b>{data.avgRatingsDaily[data.avgRatingsDaily.length - 1].avg.toFixed(2)}</b>),
                weighted rating (calculated via regression) would have been:{' '}
                <b>{data.wgtRatingsDaily[data.wgtRatingsDaily.length - 1].weightedRating.toFixed(2)}</b>
                <br/>
                Based on rating changes from <b>{data.earliestRatingChangeDate}</b> to{' '}
                <b>{data.latestRatingChangeDate}</b>.
                <br/>
                With <b>0.5</b> max downside, <b>0.5</b> max upside per day and <b>{rollingNum}</b> rolling num,
                the prices predicted would be as follows:
            </p>
            <table>
                <thead>
                    <tr>
                        <th>type</th>
                        <th>{regrStartDate}&nbsp;&nbsp;&nbsp;&nbsp;</th>
                        <th>{data.actualStart.dateString}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</th>
                        <th>{data.actualEnd.dateString}&nbsp;&nbsp;&nbsp;&nbsp;</th>
                        <th>% up/dn (from {data.actualStart.dateString})&nbsp;&nbsp;&nbsp;&nbsp;</th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{avgEnd}</td>
                        <td>{avgPct}</td>
                        <td><b>{pctDiff(actualEndPrice, avgEnd)}</b></td>
                        <td><b>{pctDiff(rollingPriceCheck, avgEnd)}</b></td>
                    </tr>
                    <tr>
                        <td>wgt ratings</td>
                        <td>{rollingRegrStart}</td>
                        ...
                        <td>{altRegrEnd}</td>
                        <td>{altRegrPct2}</td>
                        <td><b>{pctDiff(actualEndPrice, altRegrEnd)}</b></td>
                        <td><b>{pctDiff(rollingPriceCheck, altRegrEnd)}</b></td>
                    </tr>
                </tbody>
            </table>
