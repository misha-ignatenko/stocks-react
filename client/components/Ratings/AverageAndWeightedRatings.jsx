...
function AverageAndWeightedRatings({
            ...
            Meteor.call(
                ...
                (err2, earliestChange) => {
                    if (
                        ...
                        const rollingPx = Utils.stockPrices.getSimpleRollingPx(
                            pricesResult,
                            earliestChange,
                            pxRollingDays,
                        );
    ...
    useEffect(() => {
        ...
        Meteor.call(
            ...
            {
                symbol,
                startDate: avgRatingStartDate,
                endDate: avgRatingEndDate,
            },
            (err, res) => {
                if (!err) {
    ...
    const changingStart = (date) => {
        const newDate = StocksReact.ui.getStateForDateRangeChangeEvent(
            false,
            date,
        );
    ...
    const graphData = useTracker(() => {
        ...
        const ratingScalesHandle =
            StocksReact.functions.getRatingScalesHandleFromAvailableRatingChanges(
                ratingChanges,
            );
        ...
        const pricesWithNoAdjClose = _.filter(
            allStockPrices,
            (price) => !price["adjClose"],
        );
        if (pricesWithNoAdjClose.length > 0) {
            console.log(
                "ERROR, these price dates do not have adjClose: ",
                _.pluck(pricesWithNoAdjClose, "dateString"),
            );
        }
        ...
        const startDateForSubscription = currentUser
            ...
            : moment(new Date().toISOString())
                  .subtract(
                      settings.clientSettings.upcomingEarningsReleases
        ...
        const averageAnalystRatingSeries =
            Utils.ratingChanges.generateAverageAnalystRatingTimeSeries(
                symbol,
                startDateForSubscription,
                avgRatingEndDate,
                ratingChanges,
        ...
        const avgRatingsSeriesEveryDay =
        ...
        const predictionsBasedOnAvgRatings =
        ...
        const predictionsBasedOnWeightedRatings =
            Utils.ratingChanges.predictionsBasedOnRatings(
        ...
        console.log(
            "objToGraph.avgAnalystRatingsEveryDay:",
            objToGraph.avgAnalystRatingsEveryDay,
        );
    ...
    const renderAvgAnalystRatingsGraph = () => {
        ...
        return (
            <div>
                <span>
                    price reaction delay for rating changes (in days):{" "}
                    {priceReactionDelayDays}{" "}
                </span>
                <button
                    type="button"
                    ...
                    onClick={() =>
                        setPriceReactionDelayDays((prev) => prev - 1)
                    }
                >
                    -
                </button>
                ...
                <button
                    className="btn btn-light"
                    onClick={() => setStepSizePow((prev) => prev - 1)}
                >
                    -
                </button>
                <button
                ...
                <div className="col-md-12 individualStockGraph">
                    <StocksGraph
                        stocksToGraphObjects={graphData.stocksToGraphObjs}
                    />
                </div>
            </div>
        );
        ...
        return earningsReleases.map((release, index) => {
            ...
            return (
                <div key={key}>
                    <h1>This quarter</h1>
                    <h1>
                        Next earning release date:{" "}
                        {release.reportDateNextFiscalQuarter} (
                        {sourceFlag === 1
                            ? "Company confirmed"
                            : sourceFlag === 2
                    ...
                    <h1>
                        Expected EPS: {release.epsMeanEstimateNextFiscalQuarter}
                    </h1>
                    <br />
                    <h3>
                        Previous quarter EPS:{" "}
                        {release.epsActualPreviousFiscalQuarter}
                    </h3>
                    <h3>
                        EPS a year ago:{" "}
    ...
    const isReady =
        allStockPrices?.[0]?.symbol === symbol && !ratingChangesLoading;
