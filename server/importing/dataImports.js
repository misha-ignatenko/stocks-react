...
Meteor.methods({
    ...
    importEarningsReleasesRecursively: function (symbols, scheduledDataPullFlag) {
        ...
        if (symbols && symbols.length > 0) {
            Meteor.call("importEarningsRelease", symbols[0], function (error, result) {
                if (result && result.message) {
                    symbols.shift();
                    Meteor.call("importEarningsReleasesRecursively", symbols, scheduledDataPullFlag);
                }
            });
        } else {
    ...
    importEarningsRelease: function (symbol) {
        var _result = {};
        console.log("import earnings release for: ", symbol);

        var _universalSymbol = _getUniversalSymbolFromEarningsReleaseSymbol(symbol);
        var _quandlSymbol = _getEarningsReleaseSymbolFromUniversalSymbol(symbol);
        ...
        if (_canPullAgainFromQuandl(_universalSymbol)) {
            ...
            try {
                ...
                if (_i === 14) {
                        ...
                        if (!_matchingEarningsReleaseId) {
                            ...
                            if (_previousAsOfField !== _latestAsOfField) {
                                EarningsReleases.update(
                                    ...
                                    {
                                        $set: _.extend({asOf: _objectFromQuandlMyDbFormat.asOf}, _lastMod)
                                    }
                                );
                            }
                        }
                    ...
                    _result.message = "got obj from quandl";
                } else {
                    _result.message = "ERROR. _i is less than 14.";
                }
                ...
                _result.message = _err.message;
                return _result;
            }
        } else {
            _result.message = "cannot import again, yet";
            return _result;
        };
    },
