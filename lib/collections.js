...
export const EarningsReleases = new Meteor.Collection("earningsReleases", {
    transform(doc) {
        return {
            ...
            async getPriorConfirmedRelease(quartersBack = 1) {
                if (quartersBack < 1) {
                    return this;
                }
                ...
                const prior = await EarningsReleases.findOneAsync(query, {
                    sort: { asOf: -1 },
                });
                if (!prior) {
                    return;
                }
                return prior.getPriorConfirmedRelease(quartersBack - 1);
            },
