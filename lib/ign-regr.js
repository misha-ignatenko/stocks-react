...
IgnRegressionUtil.functions = {
    multiple_regression_gradient_descent: function (featureMartix, actualOutput, initialWeights, stepSize, tolerance, maxIter) {
        ...
        while (!_converged) {
            _iter++;
            var _predictions = IgnRegressionUtil.utilities.get_dot_product_arrayOfArrays_and_array(featureMartix, _weights);
            var _errors = IgnRegressionUtil.utilities.subtract_arrays(_predictions, actualOutput);

            var _gradientSumSquares = 0;
            _weights.forEach(function(weight, featureIndex) {
                var _derivative = 2 * IgnRegressionUtil.utilities.get_dot_product_two_arrays(
                        _errors,
                        IgnRegressionUtil.utilities.get_column_from_array_of_arrays(featureMartix, featureIndex)
                    );
    ...
    , multiple_regression_gradient_descent2: function (
        ...
        while (!_converged) {
            ...
            featureMartix.forEach(function(featureMatrixRow, index) {
                if (index > 0) {
                    var _weightedAvgRatingForDay = IgnRegressionUtil.utilities.get_dot_product_two_arrays(featureMatrixRow, _weights);
                    var _dayIncrease;
            ...
            var _errors = IgnRegressionUtil.utilities.subtract_arrays(_alternativePredictions, actualOutput);

            var _gradientSumSquares = 0;
            _weights.forEach(function(weight, featureIndex) {

                var _new = _.map(IgnRegressionUtil.utilities.get_column_from_array_of_arrays(featureMartix, featureIndex), function (r) {
...
IgnRegressionUtil.utilities = {
    ...
    get_dot_product_arrayOfArrays_and_array: function(arrayOfArrays, array) {
        ...
        for (var row = 0; row < arrayOfArrays.length; row++) {
            if (IgnRegressionUtil.utilities.get_row_from_array_of_arrays(arrayOfArrays, row).length !== array.length) {
        ...
        if (!_allDimensionsMatch) {
            ...
            arrayOfArrays.forEach(function(row) {
                _result.push(IgnRegressionUtil.utilities.get_dot_product_two_arrays(row, array))
            });
        }
