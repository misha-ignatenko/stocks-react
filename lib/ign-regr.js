...
const IgnRegressionUtil = {};

IgnRegressionUtil.functions = {
    multiple_regression_gradient_descent: function (
        featureMartix,
        actualOutput,
        initialWeights,
        stepSize,
        ...
        while (!_converged) {
            _iter++;
            var _predictions =
                IgnRegressionUtil.utilities.get_dot_product_arrayOfArrays_and_array(
                    featureMartix,
                    _weights,
                );
            var _errors = IgnRegressionUtil.utilities.subtract_arrays(
                _predictions,
                actualOutput,
            );
            ...
            _weights.forEach(function (weight, featureIndex) {
                var _derivative =
                    2 *
                    IgnRegressionUtil.utilities.get_dot_product_two_arrays(
                        _errors,
                        IgnRegressionUtil.utilities.get_column_from_array_of_arrays(
                            featureMartix,
                            featureIndex,
                        ),
                    );
    ...
    multiple_regression_gradient_descent2: function (
        ...
        while (!_converged) {
            ...
            featureMartix.forEach(function (featureMatrixRow, index) {
                if (index > 0) {
                    ...
                    if (_weightedAvgRatingForDay >= cutoffValue) {
                        _fractionOfMaxGoUp =
                            (_weightedAvgRatingForDay - cutoffValue) /
                            (maxRatingValue - cutoffValue);
                        _dayIncrease =
                            _fractionOfMaxGoUp * goUpPerDayAtMaxRating;
                    } else {
                        _fractionOfMaxGoDown =
                            (cutoffValue - _weightedAvgRatingForDay) /
                            (cutoffValue - minRatingValue);
                        _dayIncrease =
                            -_fractionOfMaxGoDown * goDownPerDayAtMinRating;
                    }
                    var _priceFromPreviousDay =
                        _alternativePredictions[index - 1];
            ...
            _weights.forEach(function (weight, featureIndex) {
                ...
                if (
                    _weights[featureIndex] < minPossibleWeight ||
                    _weights[featureIndex] > maxPossibleWeight
                ) {
                    _converged = true;
                }
            });

            var _totalWeight = _.reduce(
                _weights,
                function (memo, num) {
                    return memo + num;
                },
...
IgnRegressionUtil.utilities = {
    ...
    remove_column_indices_from_matrix: function (
        matrix,
        arrayOfIndicesToRemove,
    ) {
    ...
    get_dot_product_arrayOfArrays_and_array: function (arrayOfArrays, array) {
        ...
        for (var row = 0; row < arrayOfArrays.length; row++) {
            if (
                IgnRegressionUtil.utilities.get_row_from_array_of_arrays(
                    arrayOfArrays,
                    row,
                ).length !== array.length
            ) {
        ...
        if (!_allDimensionsMatch) {
            console.log(
                "ERROR, not all dimensions match for matrix and array multiplication!",
            );
        } else {
    ...
    subtract_matrices: function (matrix1, matrix2) {
        ...
        matrix1.forEach(function (matrixOneRowItem, row) {
            ...
            matrixOneRowItem.forEach(function (matrixOneColumnItem, column) {
                _subtractedRow.push(matrixOneColumnItem - matrix2[row][column]);
            });
    ...
    subtract_arrays: function (array1, array2) {
        ...
        array1.forEach(function (item, index) {
            _result.push(item - array2[index]);
        });
...
export const IgnRegression = IgnRegressionUtil;
