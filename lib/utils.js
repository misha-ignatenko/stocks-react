...
const getSetting = async (setting) => {
    ...
    const settingsObject = await Settings.findOneAsync(settingsQuery, {fields: {[setting]: 1}});
...
export const Utils = {
    ...
    getClosestPreviousWeekDayDateByCutoffTime: async function (OPTIONALrequestMomentDateTimeRightNowNyTime, format=Utils.dateFormat) {
        ...
        let _beforeCutoffTime = _timeStr < (await Utils.getTradingDayCutoffTime());
    ...
    getTradingDayCutoffTime: async function () {
        return await Utils.getCachedSetting('clientSettings.ratingChanges.fourPmInEstTimeString');
    },
    ...
    ratingChanges: {
        ...
        generateAverageAnalystRatingTimeSeries: async function(symbol, startDate, endDate, _allRatingChangesForSymbol) {
            let _ratingChangesOfInterest = _.sortBy(_allRatingChangesForSymbol.filter(r => {
                const d = r.dateString;
                return d >= startDate && d <= endDate;
            }), 'dateString');

            _ratingChangesOfInterest = await Utils.ratingChanges.excludeRatingChangesWhoseFirmsInitiatedOrSuspendedOrDroppedCoverage(_ratingChangesOfInterest);
            ...
            _uniqDates.forEach(function(uniqDate) {
                ...
                _uniqueFirms.forEach(function(firmId){
                    var _id = Utils.ratingChanges.getLatestRatingScaleIdFor(_allRatingChangesForSymbol, symbol, firmId, uniqDate);
        ...
        excludeRatingChangesWhoseFirmsInitiatedOrSuspendedOrDroppedCoverage: async function(ratingsChangesArray) {
            var _firmIdsWhoseRatingsToReject = [];

            for (const rChange of ratingsChangesArray) {
                var _newRScale = await Utils.getCachedRatingScale(rChange.newRatingId);
                var _oldRScale = await Utils.getCachedRatingScale(rChange.oldRatingId);
        ...
        async getRatingScalesForRatingChanges(ratingChanges) {
            ...
            return await RatingScales.find({_id: {$in: ratingScaleIds}}, {fields: {_id: 1, universalScaleValue: 1, researchFirmId: 1}}).fetchAsync();
        },
        ...
        generateWeightedAnalystRatingsTimeSeriesEveryDay: function(_avgRatingsSeriesEveryDay, _startDateForRegression, _endDateForRegression, historicalData, priceReactionDelayDays, priceType, _pctGoDownPerDayAtMinRating, _pctGoUpPerDayAtMaxRating, _stepSize2, _maxIter2) {
            ...
            var _data = Utils.ratingChanges.prepareDataForMultipleRegressionGradientDescentByResearchFirm(
            ...
            if (_featureMatrix && _actualOutput && _initialWeights) {
                ...
                var _preparedArrayOfWeightedRatings = Utils.ratingChanges.prepareArrayOfWeightedRatingsForGraph(_data.uniqueResearchFirmIds, _resultFromGradientDescent2.weights, _avgRatingsSeriesEveryDay);
    ...
    stockPrices: {
        getSimpleRollingPx: function (priceObjArray, endDate, rollingNum, isStrict=true) {
            ...
            const prices = Utils.stockPrices.getPricesBetween(priceObjArray, startDate, endDate);
