...
import { IgnRegression } from "./ign-regr";
...
const isBusinessDay = memoizeWithTimeout(
    (dateString) => {
...
const businessAdd = memoizeWithTimeout(
    (dateString, daysToAdd) => {
        return momentBiz(dateString)
...
const getSetting = async (setting) => {
    console.log("will go into the db", setting);
    const settingsObject = await Settings.findOneAsync(settingsQuery, {
        fields: { [setting]: 1 },
    });
...
const cache = {};
function cacheFor(fn, duration) {
    const cacheKey = [fn, duration].toString();
    cache[cacheKey] = new Map();

    return async function (...args) {
        const key = JSON.stringify(args);

        if (_.last(args)?.doRefresh) {
            cache[cacheKey].delete(key);
        }

        const cachedValue = cache[cacheKey].get(key);
        if (cache[cacheKey].has(key)) {
            return cachedValue;
        } else {
...
const avgDaysInYear = (365 * 3 + 366 * 1) / 4;
const avgDaysInQuarter = avgDaysInYear / 4;
...
export const Utils = {
    ...
    cacheFor,
    getAllCache,
    getCachedSetting: cacheFor(getSetting, 5 * 60 * 1000),
    ...
    getMinMaxFromArrOfObj: function (arrOfObj, key) {
        var _max = -1000000000.0;
        var _min = 1000000000.0;
    ...
    getClosestPreviousWeekDayDateByCutoffTime: async function (
        OPTIONALrequestMomentDateTimeRightNowNyTime,
        format = Utils.dateFormat,
    ) {
        let requestMomentDateTimeRightNowNyTime =
            OPTIONALrequestMomentDateTimeRightNowNyTime ||
            moment().tz("America/New_York");

        var _alternativeDateTime = requestMomentDateTimeRightNowNyTime.format(
            "YYYY-MM-DD HH:mm:ss",
        );
        var _timeStr = _alternativeDateTime.substring(
            11,
            _alternativeDateTime.length,
        );

        let _beforeCutoffTime =
            _timeStr < (await Utils.getTradingDayCutoffTime());
        ...
        if (_dayOfWeek === 1 && _beforeCutoffTime === true) {
            ...
            _beforeCutoffTime === true &&
            [2, 3, 4, 5].indexOf(_dayOfWeek) > -1
        ) {
            _dayDiffForEndDate = 1;
        } else {
        ...
        let _avgRatingEndDate = requestMomentDateTimeRightNowNyTime
            .subtract(_dayDiffForEndDate, "days")
            .format(format);

        return _avgRatingEndDate;
    },
    getTradingDayCutoffTime: async function () {
        return await Utils.getCachedSetting(
            "clientSettings.ratingChanges.fourPmInEstTimeString",
        );
    },
    ...
    addCalendarDays(YYYYMMDD, daysToAdd) {
        return +moment(YYYYMMDD.toString(), Utils.dateFormatYYYYMMDD)
            .add(daysToAdd, "days")
            .format(Utils.dateFormatYYYYMMDD);
    },
    dateRangeAround(referenceYYYYMMDD, spreadDays = 30) {
        return {
            $gte: Utils.addCalendarDays(referenceYYYYMMDD, -spreadDays),
            $lte: Utils.addCalendarDays(referenceYYYYMMDD, spreadDays),
        };
    },
    ...
    getCachedRatingScale: cacheFor(
        async (id) => {
            return await RatingScales.findOneAsync(id);
        },
        24 * 60 * 60 * 1000, // 24 hours
    ),
    ratingChanges: {
        generateAverageAnalystRatingTimeSeries: async function (
            symbol,
            startDate,
            endDate,
            _allRatingChangesForSymbol,
        ) {
            let _ratingChangesOfInterest = _.sortBy(
                _allRatingChangesForSymbol.filter((r) => {
            ...
            _ratingChangesOfInterest =
                await Utils.ratingChanges.excludeRatingChangesWhoseFirmsInitiatedOrSuspendedOrDroppedCoverage(
                    _ratingChangesOfInterest,
                );
            console.log(
                "generateAverageAnalystRatingTimeSeries",
                symbol,
                startDate,
                endDate,
            ...
            var _uniqueFirms = _.uniq(
                _.pluck(_ratingChangesOfInterest, "researchFirmId"),
            );
            ...
            _uniqueFirms.forEach(function (uniqueFirmId) {
                ...
                while (i < _ratingChangesOfInterest.length) {
                    if (
                        _ratingChangesOfInterest[i].researchFirmId ===
                        uniqueFirmId
                    ) {
                        _zeroSeries.push(
                            _ratingChangesOfInterest[i].oldRatingId,
                        );
                        break;
                    }
                    i++;
            ...
            var _uniqDatesAddition = _.uniq(
                _.pluck(_ratingChangesOfInterest, "date"),
            );
            _uniqDates = _uniqDates.concat(
                _.pluck(_ratingChangesOfInterest, "date"),
            );

            //figure out date for finalSeries
            var _dateForFinalSeries;
            var _today = moment(new Date()).format(this.dateFormat);
            ...
            _uniqDates.forEach(function (uniqDate) {
                ...
                _uniqueFirms.forEach(function (firmId) {
                    var _id = Utils.ratingChanges.getLatestRatingScaleIdFor(
                        _allRatingChangesForSymbol,
                        symbol,
                        firmId,
                        uniqDate,
            ...
            for (const res of _result) {
                ...
                for (const ratingScaleId of res.ratingScalesIds) {
                    var _rSc = await Utils.getCachedRatingScale(ratingScaleId);
                    if (
                        _rSc.universalScaleValue ===
                            "beforeCoverageInitiatedString" ||
                        _rSc.universalScaleValue === "coverageDroppedString" ||
                        _rSc.universalScaleValue ===
        ...
        excludeRatingChangesWhoseFirmsInitiatedOrSuspendedOrDroppedCoverage:
            async function (ratingsChangesArray) {
                var _firmIdsWhoseRatingsToReject = [];

                for (const rChange of ratingsChangesArray) {
                    var _newRScale = await Utils.getCachedRatingScale(
                        rChange.newRatingId,
                    );
                    var _oldRScale = await Utils.getCachedRatingScale(
                        rChange.oldRatingId,
                    );
                    var _newRScaleValue = _newRScale.universalScaleValue;
                    var _oldRScaleValue = _oldRScale.universalScaleValue;
                    if (
                        _newRScaleValue === "beforeCoverageInitiatedString" ||
                        ...
                        _firmIdsWhoseRatingsToReject.push(
                            _newRScale.researchFirmId,
                        );
                        _firmIdsWhoseRatingsToReject.push(
                            _oldRScale.researchFirmId,
                        );
                    }
                }
                _firmIdsWhoseRatingsToReject = _.uniq(
                    _firmIdsWhoseRatingsToReject,
                );

                if (_firmIdsWhoseRatingsToReject.length > 0) {
                    ratingsChangesArray = _.reject(
                        ratingsChangesArray,
                        function (ratingsChange) {
                            return (
                                _firmIdsWhoseRatingsToReject.indexOf(
                                    ratingsChange.researchFirmId,
                                ) > -1
                            );
                        },
                    );
        ...
        async getRatingScalesForRatingChanges(ratingChanges) {
            ...
            return await RatingScales.find(
                { _id: { $in: ratingScaleIds } },
                {
                    fields: {
                        _id: 1,
                        universalScaleValue: 1,
                        researchFirmId: 1,
                    },
                },
            ).fetchAsync();
        ...
        getLatestRatingScaleIdFor: function (
            allRatingChanges,
            symbol,
            researchFirmId,
            requestedDate,
            ...
            var _ratingChanges = _.filter(allRatingChanges, function (rCh) {
                return (
                    rCh.researchFirmId === researchFirmId &&
                    rCh.symbol === symbol
                );
            });
            var rDate = moment(new Date(requestedDate)).format(this.dateFormat);

            var _ratingScaleId;
            if (_ratingChanges && _ratingChanges.length > 0) {
                var _onOrBeforeUnsorted = _.filter(
                    _ratingChanges,
                    function (rChange) {
                        var _extractedDateStringNoTimezone = moment(
                            new Date(rChange.date),
                        ).format(_format);
                        return (
                            moment(_extractedDateStringNoTimezone).isSame(
                                rDate,
                            ) ||
                            moment(_extractedDateStringNoTimezone).isBefore(
                                rDate,
                            )
                        );
                    },
                );
                var _afterUnsorted = _.filter(
                    _ratingChanges,
                    function (rChange) {
                        var _extractedDateStringNoTimezone = moment(
                            new Date(rChange.date),
                        ).format(_format);
                        return moment(_extractedDateStringNoTimezone).isAfter(
                            rDate,
                        );
                    },
                );
                var _onOrBeforeSorted = _.sortBy(
                    _onOrBeforeUnsorted,
                    function (obj) {
                ...
                if (_onOrBeforeSorted && _onOrBeforeSorted.length > 0) {
                    ...
                    _ratingScaleId =
                        _onOrBeforeSorted[_onOrBeforeSorted.length - 1]
                            .newRatingId;
        ...
        generateAverageAnalystRatingTimeSeriesEveryDay: function (
            ...
            if (_averageAnalystRatingSeries.length > 1) {
                ...
                historicalData.forEach(function (histData, index) {
                    var _avgRatingDateInProgress = new Date(
                        _averageAnalystRatingSeries[_avgRatingIndexInProgress]
                            .date,
                    )
                        .toISOString()
                        .substring(0, 10);
                    var _date = moment(histData.date)
                        .tz("America/New_York")
                    ...
                    if (moment(_date).isBefore(_avgRatingDateInProgress)) {
                        ...
                        _result.push(
                            _.extend(_obj1, {
                                date: histData.date,
                                dateString: histData.dateString,
                            }),
                        );
                    } else {
        ...
        generateWeightedAnalystRatingsTimeSeriesEveryDay: function (
            ...
            priceType,
            _pctGoDownPerDayAtMinRating,
            _pctGoUpPerDayAtMaxRating,
            _stepSize2,
            _maxIter2,
        ) {
            var _result = {};
            ...
            var _data =
                Utils.ratingChanges.prepareDataForMultipleRegressionGradientDescentByResearchFirm(
                    _avgRatingsSeriesEveryDay,
                    historicalData,
                    _startDateForRegression,
                    _endDateForRegression,
            ...
            if (_featureMatrix && _actualOutput && _initialWeights) {
                ...
                var _resultFromGradientDescent2 =
                    IgnRegression.functions.multiple_regression_gradient_descent2(
                        _featureMatrix,
                        _actualOutput,
                ...
                _.each(_data.uniqueResearchFirmIds, function (firmId, idx) {
                    _firmsAndWeights[firmId] =
                        _resultFromGradientDescent2.weights[idx];
                });
                console.log("firms and weights: ", _firmsAndWeights);
                console.log(
                    "total iterations: ",
                    _resultFromGradientDescent2.iter,
                );

                var _preparedArrayOfWeightedRatings =
                    Utils.ratingChanges.prepareArrayOfWeightedRatingsForGraph(
                        _data.uniqueResearchFirmIds,
                        _resultFromGradientDescent2.weights,
                        _avgRatingsSeriesEveryDay,
                    );
                _result = {
                    ratings: _preparedArrayOfWeightedRatings,
                    weights: _firmsAndWeights,
                };
            }
        ...
        prepareDataForMultipleRegressionGradientDescentByResearchFirm:
            function (
                avgRatingsDataRaw,
                pricesDataRaw,
                startDateForRegr,
                ...
                avgRatingsDataRaw.forEach(function (avgRawData) {
                    var _thisDate = new Date(avgRawData.date)
                        .toISOString()
                        .substring(0, 10);
                    if (
                        !moment(_thisDate).isAfter(endDateForRegr) &&
                        !moment(_thisDate).isBefore(startDateForRegr)
                    ) {
                        _avgRatingsDataWithinDateRange.push(avgRawData);
                    }
                });
                var avgRatingsData = _avgRatingsDataWithinDateRange.slice(
                    0,
                    _avgRatingsDataWithinDateRange.length -
                        priceReactionDelayDays,
                );

                var _pricesDataWithinDateRange = [];
                pricesDataRaw.forEach(function (priceItem) {
                    var _thisDate = new Date(priceItem.date)
                        .toISOString()
                        .substring(0, 10);
                    if (
                        !moment(_thisDate).isAfter(endDateForRegr) &&
                ...
                var pricesData = _pricesDataWithinDateRange.slice(
                    priceReactionDelayDays,
                    _pricesDataWithinDateRange.length,
                );
                ...
                avgRatingsData.forEach(function (obj) {
                    if (obj.ratingScales) {
                        obj.ratingScales.forEach(function (ratingScaleInfo) {
                            if (
                                ratingScaleInfo.researchFirmId &&
                                _.indexOf(
                                    _uniqueResearchFirmIds,
                                    ratingScaleInfo.researchFirmId,
                                ) === -1
                            ) {
                                _uniqueResearchFirmIds.push(
                                    ratingScaleInfo.researchFirmId,
                                );
                            }
                        });
                ...
                avgRatingsData.forEach(function (obj) {
                    ...
                    if (obj.ratingScales) {
                        obj.ratingScales.forEach(function (ratingScaleInfo) {
                            if (
                                ratingScaleInfo.universalScaleValue &&
                                ratingScaleInfo.researchFirmId
                            ) {
                                _features[
                                    _.indexOf(
                ...
                var _initialWeightForConstant =
                    1 / _uniqueResearchFirmIds.length;
        ...
        prepareArrayOfWeightedRatingsForGraph: function (
            uniqueResearchFirmIds,
            ratingFirmsWeights,
            avgRatingsEveryDay,
        ) {
            var _result = [];
            ...
            avgRatingsEveryDay.forEach(function (obj) {
                ...
                obj.ratingScales.forEach(function (ratingScale) {
                    var _firmIndex = _.indexOf(
                        uniqueResearchFirmIds,
                        ratingScale.researchFirmId,
                    );
                    var _firmWeight = ratingFirmsWeights[_firmIndex];
                    _totaleightedRating +=
                        _firmWeight * ratingScale.universalScaleValue;
                });
                //also add the constant
                var _constantIndex = _.indexOf(
                    uniqueResearchFirmIds,
                    "constant",
                );
        ...
        predictionsBasedOnRatings: function (
            _datesAndRatings,
            pricesArr,
            priceTypeStr,
            rollingPx,
            ...
            _.each(pricesArr, function (obj, index) {
                if (index > 0) {
                    ...
                    if (_rating >= _cutoffValue) {
                        _fractionOfMaxGoUp =
                            (_rating - _cutoffValue) /
                            (_maxRatingValue - _cutoffValue);
                        _dayIncrease =
                            _fractionOfMaxGoUp * goUpPerDayAtMaxRating;
                    } else {
                        _fractionOfMaxGoDown =
                            (_cutoffValue - _rating) /
                            (_cutoffValue - _minRatingValue);
                        _dayIncrease =
                            -_fractionOfMaxGoDown * goDownPerDayAtMinRating;
                    }
    ...
    stockPrices: {
        getSimpleRollingPx: function (
            ...
            const daysToSubtract =
                rollingNum - (Utils.isBusinessDay(endDate) ? 1 : 0);
            const startDate = Utils.businessAdd(endDate, -daysToSubtract);

            const prices = Utils.stockPrices.getPricesBetween(
                priceObjArray,
                startDate,
                endDate,
            );
            ...
            if (!isStrict || rollingNum === closePrices.length) {
                ...
                console.log(
                    "there is an issue with rolling prices",
                    prices[0]?.symbol,
                    startDate,
                    endDate,
                    closePrices.length,
        ...
        getPricesBetween: function (priceObjArray, startDate, endDate) {
            var _filteredPricesArray = _.filter(
                priceObjArray,
                function (priceObjForDay) {
                    return (
                        priceObjForDay.dateString >= startDate &&
                        priceObjForDay.dateString <= endDate
                    );
                },
            );
        ...
        getPriceOnDayNoRetry(prices, dateString, priceField = "adjClose") {
            return Utils.stockPrices.getPricesBetween(
                prices,
                dateString,
                dateString,
            )[0]?.[priceField];
        ...
        getPriceOnDay(prices, dateString, priceField = "adjClose") {
            const firstAttempt = Utils.stockPrices.getPriceOnDayNoRetry(
                prices,
                dateString,
                priceField,
            );
            if (_.isUndefined(firstAttempt)) {
                ...
                return Utils.stockPrices.getPriceOnDayNoRetry(
                    prices,
                    altDate,
                    priceField,
                );
...
momentBiz.updateLocale("us", {
    ...
    holidayFormat: Utils.dateFormat,
});
