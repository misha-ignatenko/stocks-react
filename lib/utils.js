StocksReactUtils = {
    ...
    symbols: {
        getLiveSymbols: function () {
            var _allStockSymbols = _.pluck(Stocks.find({"delisted": {$exists: false}}, {fields: {_id: 1}}).fetch(), "_id");
    ...
    stockPrices: {
        getPricesBetween: function (symbol, optionalStartDate, optionalEndDate) {
            console.log("getting prices for: ", symbol, optionalStartDate, optionalEndDate);
            var _qry = [
                {symbol: symbol},
                {dateString: {$nin: StocksReactUtils.getHolidays()}}
            ];

            if (optionalStartDate) {
                _qry.push({dateString: {$gte: optionalStartDate}})
            }

            if (optionalEndDate) {
                _qry.push({dateString: {$lte: optionalEndDate}})
            }

            return NewStockPrices.find({$and: _qry}, {sort: {dateString: 1}}).fetch();
        },
    }
};