...
StocksReactUtils = {
    ...
    stockPrices: {
        getSimpleRollingPx: function (priceObjArray, endDate, rollingNum, isStrict=true) {
            const endDateMoment = momentBiz(endDate);
            const daysToSubtract = rollingNum - (endDateMoment.isBusinessDay() ? 1 : 0);
            const startDate = endDateMoment.businessAdd(-daysToSubtract).format(Utils.dateFormat);
        ...
        getPriceOnDayNoRetry(prices, dateString) {
            return Utils.stockPrices.getPricesBetween(prices, dateString, dateString)[0]?.adjClose;
        },
        getPriceOnDay(prices, dateString) {
            const firstAttempt = Utils.stockPrices.getPriceOnDayNoRetry(prices, dateString);
            if (_.isUndefined(firstAttempt)) {
                console.log('going back 1 day', dateString);
                const altDate = momentBiz(dateString).businessAdd(-1).format(Utils.dateFormat);
                return Utils.stockPrices.getPriceOnDayNoRetry(prices, altDate);
            }
            return firstAttempt;
        },
    }
